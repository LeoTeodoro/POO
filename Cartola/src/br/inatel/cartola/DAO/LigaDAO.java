...
public class LigaDAO extends ConnectionDAO {
    ...
    public boolean insertLiga(Liga liga) {
        ...
        String sql = "INSERT INTO Liga (Nome,Times) values(?,?)";
        try {
    ...
    //UPDATE
    public boolean updateLiga(int id, Liga liga) {
        connectToDB();
        String sql = "UPDATE Liga SET Nomes=?, Times=? where id=?";
        try {
            pst = con.prepareStatement(sql);
            pst.setString(1, liga.nome);
            pst.setString(2, String.valueOf(liga.times));
    ...
    //DELETE
    public boolean deleteTime(Liga liga) {
        connectToDB();
    ...
    public ArrayList<Liga> selectLigas() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                Liga ligaAux = new Liga(rs.getString("nome"),rs.getInt("id"));
